...
// Tách local part (phần số sau prefix)
...
const PhoneInput: React.FC<PhoneInputProps> = ({
    ...
    // ----------------------------
    // MEMO HOÁ GIÚP TRÁNH RE-RENDER LẶP
    // ----------------------------
    const prefix = useMemo(() => detectPrefix(value), [value]);
    const local = useMemo(() => stripPrefix(value), [value]);

    const selected = useMemo(
        () => COUNTRIES.find((c) => c.dial_code === prefix) || COUNTRIES[0],
        [prefix]
    );
    ...
    // ----------------------------
    // USER TYPING INTO LOCAL INPUT
    // ----------------------------
    const handleLocalChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const clean = e.target.value.replace(/\D/g, ""); // chỉ giữ số
        onChange?.(`${selected.dial_code} ${clean}`);
    };

    // ----------------------------
    // USER SELECTS ANOTHER COUNTRY
    // ----------------------------
    const handleSelect = (c: Country) => {
        setMenuOpen(false);
        onChange?.(`${c.dial_code} ${local}`);
    };
        ...
        <div className={`relative ${className || ""}`} ref={ref}>
            <div className="flex items-center gap-2">
                ...
                <button
                    ...
                    className="flex items-center justify-center w-10 h-10 rounded-md border border-border bg-background text-sm"
                >
                ...
                {/* Prefix */}
                <span className="text-sm text-text-light select-none">
                    {selected.dial_code}
                </span>
            ...
            {menuOpen && (
                <div className="absolute left-0 mt-2 w-80 max-h-72 overflow-auto bg-background border border-border rounded shadow z-50">
                    ...
                    <ul>
                        {list.map((c) => (
                            <li key={c.code}>
                                <button
                                    ...
                                    className="w-full text-left px-4 py-3 hover:bg-primary/10 flex items-center gap-3"
                                >
                        ...
                        {list.length === 0 && (
                            <li className="px-4 py-2 text-text-light">
